...
//
//  1. TS FIX: Merged window.gsap declaration. The Hero component declared a
//     minimal { to } type; AboutSection needs { registerPlugin, fromTo, to }.
//     Both files must declare the same shape — use the fuller union here and
//     update Hero to match (or move the declaration to a shared global.d.ts).
//
//  2. @import removed from <style>. Move font <link> tags to layout.tsx <head>
//     (see comment below). @import is parser-blocking on every render.
//
//  3. ScrollTrigger loaded in PARALLEL with GSAP (Promise.all) instead of
//     sequentially — saves ~200 ms script load time on slow connections.
//
//  4. GSAP still used for scroll-driven effects (they require JS), but the
//     IntersectionObserver defers loading until 300px before viewport.
// ─────────────────────────────────────────────────────────────────────────────

/*
  Add to app/layout.tsx <head> — removes render-blocking @import:
...
// ── Merged global GSAP types ───────────────────────────────────────────────────
// This declaration must match (or be a superset of) any other window.gsap
// declaration in the project (e.g. Hero.tsx). Keep in a shared global.d.ts
// if both components live in the same Next.js app.
declare global {
  interface Window {
    gsap: {
...
export default function AboutSection(): JSX.Element {
  ...
  const loadGSAP = async (): Promise<void> => {
    ...
    // ── PARALLEL load — saves ~200 ms vs sequential await ──────────────────
    await Promise.all([
  ...
  const initAnimations = (): void => {
    ...
    if (headingWrap) {
      gsap.fromTo(
        ...
        {
          yPercent: 0, rotationX: 0, opacity: 1, stagger: 0.025,
    ...
    ([".ci-img-main", ".ci-img-accent"] as const).forEach((sel, i) => {
      ...
      if (cover) {
        gsap.fromTo(cover, { scaleX: 1 }, {
          scaleX: 0, duration: 1.3, delay: i * 0.2, ease: "power4.inOut",
    ...
    if (badge) {
      gsap.fromTo(badge, { scale: 0, rotation: -15 }, {
    ...
    gsap.fromTo(
      ...
      { x: 70, opacity: 0 },
      {
        ...
        scrollTrigger: {
          trigger: sec.querySelector<HTMLElement>(".ci-pillars") ?? sec,
          start: "top 80%",
        },
      }
    );
  ...
  return (
    <>
      <style>{`
        /*
          Fonts omitted here — add to layout.tsx <head> as shown above.
          @import inside <style> is parser-blocking and hurts FCP.
        */
        ...
        .ci-marquee-item {
          ...
          gap: 18px; padding: 0 28px; flex-shrink: 0;
          font-family: 'Bebas Neue', sans-serif;
          font-size: 1rem; letter-spacing: 0.14em; color: var(--ci-muted);
        }
        .ci-marquee-dot {
        ...
        @media (max-width: 960px) {
          .ci-split { grid-template-columns: 1fr; gap: 60px; padding: 80px 0; }
        }
        ...
        .ci-eyebrow-label {
          ...
          text-transform: uppercase; color: var(--gold); font-family: 'DM Sans', sans-serif;
        }
        .ci-line-draw {
        ...
        .ci-heading-line {
          ...
          line-height: 0.92; letter-spacing: 0.025em; color: var(--ci-warm);
        }
        ...
        .ci-body-text {
          ...
          color: rgba(240,234,214,0.90); max-width: 500px; margin-bottom: 24px;
          font-style: italic;
        ...
        .ci-img-accent .ci-img-el { object-fit: cover; display: block; z-index: 1; }
        ...
        .ci-img-label {
          ...
          color: var(--gold); z-index: 4; backdrop-filter: blur(8px);
        }
        ...
        .ci-pillars {
          ...
          gap: 1px; background: var(--ci-border);
        }
        ...
        .ci-pillar-bg-num {
          ...
          transition: color 0.45s ease; pointer-events: none; user-select: none;
        }
        ...
        .ci-pillar-body {
          ...
          line-height: 1.85; color: rgba(240,234,214,0.90); font-weight: 400;
        }

        .ci-bottom-rule {
          height: 1px; background: rgba(255,255,255,0.07); position: relative; z-index: 2;
        }

        @media (prefers-reduced-motion: reduce) {
          .ci-marquee-track { animation: none; }
        }
      `}</style>
